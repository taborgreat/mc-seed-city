...
/**
 * Exhibit poses for screenshots and showcases (Mcdrizzy, PR #1): a mob whose custom name is
 * {@code SC:<pose>} (walk, run, look, fly, build, carry, repair, sleep, alert, attack) loops
 * that pose on the client. Ordinary city mobs, which have no such name, are never touched.
 */
public final class ShowcaseAnimation {
	...
	public static void apply(LivingEntity entity, LivingEntityRenderState state) {
		...
		if (state instanceof BuilderRenderState b) {
			b.building = pose.equals("build");
			b.carrying = pose.equals("carry");
			b.grounded = !pose.equals("fly");
			b.flightSpeed = b.grounded ? 0 : .18F;
		}
		if (state instanceof RectifierRenderState r) {
			r.repairing = pose.equals("repair");
			r.grounded = !pose.equals("fly");
		...
		if (state instanceof SentinelRenderState s) {
			...
			s.jogging = pose.equals("run");
			s.attackTime = pose.equals("attack") ? (t % 20) / 20 : 0;
		}
	}
}
