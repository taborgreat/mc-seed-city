...
document.querySelectorAll('[data-pose]').forEach(b=>b.onclick=()=>{pose=b.dataset.pose;document.querySelectorAll('[data-pose]').forEach(x=>x.classList.toggle('active',x===b));document.getElementById('poseLabel').textContent={idle:'Resting',carry:'Slithering'}[pose]});
...
document.getElementById('saveViewer').onclick=()=>save(new Blob([standaloneDocument],{type:'text/html'}),'Seed-City-Redstone Rat.html');
document.getElementById('saveImage').onclick=()=>canvas.toBlob(b=>{if(b)save(b,'Redstone Rat-'+pose+'-preview.png')});
...
function frame(now){const dt=Math.min((now-previous)/1000,.05);previous=now;if(play.checked)time+=dt*20;if(spin.checked&&!drag)yaw+=dt*.3;
 const w=canvas.clientWidth,h=canvas.clientHeight;renderer.setSize(w,h,false);const half=Math.max(16,18*h/w)/zoom;camera.left=-half*w/h;camera.right=half*w/h;camera.top=half;camera.bottom=-half;camera.updateProjectionMatrix();camera.position.set(Math.sin(yaw)*70,20-Math.sin(pitch)*70,-Math.cos(yaw)*70);camera.lookAt(0,20,0);
 const moving=pose==='carry';
 const phase=time*(moving?.55:.08),strength=moving?1:.08;
 groups.head.rotation.x=document.getElementById('lookUp').checked?-.9:0;
 groups.head.rotation.y=document.getElementById('lookUp').checked?Math.sin(time*.025)*.2:0;
 groups.head.rotation.y+=moving?Math.cos(phase)*.055:0;
 groups.head.rotation.x+=moving?Math.sin(phase*2)*.025:0;
 groups.head.position.x=moving?Math.sin(phase)*.10:0;
 for(let i=2;i<7;i++){
   const part=groups['segment_'+i];
   part.rotation.y=Math.cos(phase-i*.65)*.07*(1+Math.abs(i-2)*.35)*strength;
   part.position.x=Math.sin(phase-i*.65)*.14*strength;
 }
 let pawIndex=0;
 for(const side of ['left','right'])for(const end of ['front','back']){
   const name='paw_'+side+'_'+end, part=groups[name], base=specGroups[name].pivot;
   const pawPhase=phase*2+(pawIndex===0||pawIndex===3?0:Math.PI),step=Math.sin(pawPhase);
   part.position.z=base[2]+(moving?step*.36:0);
   part.position.y=base[1]-24-(moving?Math.max(0,(-Math.cos(pawPhase)-.3)/.7)*.18:0);pawIndex++;
 }
 for(let i=0;i<3;i++){
   const part=groups['tail_'+i];
   part.position.x=Math.sin(phase-4.5-i*.5)*(.30+i*.18)*strength;
   part.rotation.y=Math.cos(phase-4.5-i*.5)*.20*strength;
 }
